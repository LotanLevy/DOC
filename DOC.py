...
from losses_and_metrices import  compactnessLoss, FeaturesLoss
...
def train(model, train_gens, steps_per_epoch, validation_data, validation_steps,
                               epochs, print_freq=10, summary_writer=None):
        ...
        for _ in range(steps_per_epoch):
            ...
            if count % print_freq == 0:
                ...
                for _ in range(validation_steps):
                    val_output = model.test_step(ref_inputs, ref_labels, tar_inputs, tar_labels)
...
def main():
    ...
    features_loss = FeaturesLoss(np.concatenate([tar_train_datagen.next()[0] for i in range(20)]), model)
    ...
    metrics = {"accuracy": tf.keras.metrics.CategoricalAccuracy, "total":tf.keras.metrics.Mean}
    ...
    train(model, (ref_train_datagen, tar_train_datagen), args.iter, (ref_val_datagen, tar_val_datagen), args.val_size,
          1, print_freq=args.print, summary_writer=summary_writer)

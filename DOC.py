...
def get_args():
    ...
    parser.add_argument('--test_size', type=int, default=10, help='number of alien and target examples for testing')

    parser.add_argument('--templates_num', type=int, default=10)




    parser.add_argument('--ref_batch_size', '-rb', type=int, default=32, help='number of batches')
    parser.add_argument('--tar_batch_size', '-tb', type=int, default=2, help='number of batches')
...
def get_dataset(datagen, batchs):
    images = []
    labels = []
...
def train(model, train_gens, steps_per_epoch, validation_data, validation_steps,
    ...
    ckpt_path = os.path.join(output_path, "ckpts/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))

    ref_train_gen, tar_train_gen = train_gens

    ref_val_gen, tar_val_gen = validation_data

    val_ref_inputs, val_ref_labels = get_dataset(ref_val_gen, validation_steps)

    val_tar_inputs, val_tar_labels = get_dataset(tar_train_gen, validation_steps)
    ...
    for epoch in range(epochs):
        ...
        for _ in range(steps_per_epoch):
            ...
            if count % print_freq == 0:
                ...
                for i in range(len(val_ref_inputs)):
...
def main():
    ...
    templates_images = np.concatenate([tar_train_datagen.next()[0] for i in range(args.templates_num)])

    features_loss = FeaturesLoss(templates_images, model, args.cls_num, args.tar_batch_size)
    ...
    if args.alien_path is not None:
        target_data = np.concatenate([tar_val_datagen.next()[0] for _ in range(int(args.test_size/args.tar_batch_size))])
        ...
        aoc_helper = AOC_helper(templates_images, target_data, alien_data, args.cls_num, args.tar_batch_size)

        print("target_test {} alien size {}".format(len(target_data), len(alien_data)))
